...
def get_all_ai_summaries(page_number: int = 1, page_size: int = 50):
    """Fetch paginated rows from ai_summary as a list of dicts.

    Returning all rows can be extremely slow and can cause gateway timeouts
    when the table grows large. Pagination ensures the API returns quickly.
    """
    connection = get_connection()
    ...
    try:
        ...
        print(f"Fetched {len(rows)} summaries from database (page {page_number})")
        return rows
